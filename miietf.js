...
async function getAppWriteData(productName) {
    ...
    for (let attempt = 0; attempt < retryLimit; attempt++) {
        try {
            ...
            if (productName === 'MIIETF') {
                appw_data = await fetchWithTimeout('https://api.app.mahaana.com/marketfeedinvestuniverse/api/fund-data?name=miietf', timeoutDuration);
            } 
            else if (productName === 'MICF') {
                appw_data = await fetchWithTimeout('https://api.app.mahaana.com/marketfeedinvestuniverse/api/fund-data?name=micf', timeoutDuration);
            }
            else if (productName === 'MIIRF') {
                console.log('Fetching MIIRF data');
                appw_data = await fetchWithTimeout('https://api.app.mahaana.com/marketfeedinvestuniverse/api/fund-data?name=miirf', timeoutDuration);
            }
...
function getFundPrices(productName, appw_price) {
    ...
    if (productName === "MIIETF"){
        latest_nav = appw_price[appw_price.length - 1].nav_adjusted
    }
    else{
        latest_nav = appw_price[appw_price.length - 1].nav
    }
