...
let base = new Airtable({
    apiKey: 'patnDPQnOez6XuH3I.acbafbff38cb2659ad2a74247aa50db04dc276aaccda314aedf7df118f6bf3e2'
}).base('app9fpjsdlh5R7gsq');
...
const createText = (elementId,content)=>{
...
function renderLoop(data, airPerformances) {
    ...
    const dataMappingsUpdated = [{
        elementClass: '.assetallocation-list',
        data: currentAssetAllocation
    }, {
        elementClass: '.credit-list',
        data: creditRating
    }, {
        elementClass: '.holding-list',
        data: holding
    }];
...
async function fetchData(airPerformances) {
    ...
    try {
        ...
        // MAHANANA BACKEND //
        //////////////////////
        const response = await fetch(`${mahaanaWealthCashFund}/api/CashFund/miietf`);
        if (!response.ok) {
        ...
        Array.from(navDateElement).forEach(element=>{
            element.textContent = "as of" + " " + moment(navDate, 'YYYY-MM-DD').format('D MMM YYYY')
        }
        );
...
function getFormattedDate(date) {
    const navDate = moment(date, "DDMMYYYY").format('DD MMM YYYY');
    return "as of " + navDate
}
...
function displayReports(reportsData) {
    ...
    if (reportsBodyContainer) {
        displayedData.forEach((data)=>{
            ...
            //     </a>`;
            row.innerHTML = html;
            reportsBodyContainer.appendChild(row)
        }
        )
    }
...
function goToPage(page) {
    if (page >= 1 && page <= Math.ceil((reportsData.length || 0) / itemsPerPage)) {
        currentPage = page;
        window.currentPage = displayReports(reportsData)
    }
}
const graphDurs = [{
    key: '1Y',
